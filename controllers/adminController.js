...
export const getTrustedDevices = async (req, res) => {
  try {
    ...
    const admin = await Admin.findById(adminId).select("trustedDevices lastLoginAt");
    if (!admin) return res.status(404).json({ message: "Admin not found" });

    return res.json({
      success: true,
      trustedDevices: admin.trustedDevices.map(d) => ({
        fingerprint: d.fingerprint,
        deviceInfo: d.deviceInfo || "Unknown",
        addedAt: d.addedAt,
        ipAtTrust: d.ipAtTrust || "N/A",
        isCurrent: d.fingerprint === req.body.deviceFingerprint, // optional: highlight current
      })),
      total: admin.trustedDevices.length,
    });
  } catch (err) {
...
export const removeTrustedDevice = async (req, res) => {
  ...
  try {
    ...
    if (result.modifiedCount === 0) {
      return res.status(404).json({ message: "Device not found or already removed" });
    }
    ...
    await LoginAudit.create({
      ...
      success: true,
      reason: `removed trusted device: ${fingerprint.substring(0, 12)}...`,
    });

    return res.json({ success: true, message: "Device removed from trusted list" });
  } catch (err) {
